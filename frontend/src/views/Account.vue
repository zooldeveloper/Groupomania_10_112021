...
<template>
	<div class="about">
		<Header />
		<ServerMsg v-if="successResMsg !== null" :successResMsg="successResMsg" />
		<h1>Account</h1>
		<UserProfile :userImage="
			currentUser.imageUrl ? currentUser.imageUrl : userImage
		" :userFirstName="currentUser.firstName" :userLastName="currentUser.lastName" :userJobTitle="currentUser.jobTitle"
			:userSubscribersNum="currentUser.subscribersNum" :userEmail="currentUser.email"
			:userBio="currentUser.bio" :isAcualUser="true" @trigger-user-prfile-setting="onUserPorfileSetting" />
			...
			<div class="inner-wrapper" v-show="userProfileSetting">
				...
				<section id="userinfo">
					<div class="userinfo__user userinfo__user-border">
						...
						<ImagePreview v-if="myFile !== null" class="image-preview"
							:imagePreview="imagePreview" customClasses="imagepreview-user-profile"
							@trigger-on-cancel="
								removeImagePreview
							" />
						<Button v-if="myFile !== null" name="Send It" class="user-profile-image-btn"
							@click="onSubmit" />
					</div>
					<div class="userinfo__user">
						<form class="userinfo__bioform" @submit.prevent="onSubmit">
							<div class="userinfo__formgroupe">
								<input class="
										userinfo__input-jobtitle
									" type="text" v-model="jobTitleToSave" placeholder="Your job title!"
									aria-label="A text field for adding or modifying user's job title" />
								<textarea class="
										userinfo__textarea-bio
									" placeholder="Your bio!" v-model="bioToSave" aria-label="A text field for adding or modifying user's bio">
								</textarea>
							</div>
							<button class="userinfo__btn" type="submit" aria-label="Send your bio">
								<font-awesome-icon icon="paper-plane" color="#2b7b85" size="lg" />
							</button>
						</form>
						<p v-if="currentUser.isAdmin == 'true'">
							{{ currentUser.firstName }}
							{{ currentUser.lastName }} is <span>the Admin</span>
						</p>
					</div>
				</section>
				<section id="usersetting" :class="[
					...
					<div class="usersetting__setting">
						...
						<form @submit.prevent="onSubmit">
							<div class="usersetting__form-group">
								...
								<small v-if="checkOldPassword">{{ oldPasswordError
								}}</small>
							</div>
							<div class="usersetting__form-group">
								<span>
								...
								<small v-if="checkNewPassword">{{ newPasswordError
								}}</small>
							</div>
							<Button name="Submit" classes="btn" />
						</form>
					</div>
					<div class="usersetting__setting">
						<h2>Delete your account</h2>
						<Button name="Delete" classes="btn custom-btn" @click="onDeleteAccount" />
						<Button name="I confirm" classes="btn custom-btn" :class="[
							deleteAccount
		...
		<main>
			...
			<section id="userpost" v-for="post in posts" :key="post.post_id">
				<div class="userpost__wrapper" v-if="post.id === user[0].id">
					<UserPost :imageUrl="post.imageUrl !== null ? post.imageUrl : userImage" 
						:post_id="post.post_id" :firstName="post.firstName" :lastName="post.lastName" :postDate="post.creation_date"
						:textualPost="post.textual_post" 
						:isPostIdEqualToUserIdOrIsAdmin="post.id === user[0].id ||currentUser.isAdmin == 'true'" 
						:isPostIdEqualToUserId="post.id === user[0].id" 
						:isPostImageUrlNotUndefined="post.image_url != undefined" 
						:postImage="post.image_url" :likes="post.likes" :disLikes="post.dislikes" :comments="post.comments" />
				</div>
			</section>
	...
	methods: {
		...
		onSubmit() {
			if (this.myFile != null) {
				...
				this.newPasswordError = "Your new password required!";
				...
				this.newPasswordError = "At least 8 characters!";
				this.isNewPasswordNotValid = true;
				this.isOldPasswordNotValid = false;
			} else if (
				this.oldPassword === null &&
				...
				this.oldPasswordError = "Your old password required!";
				this.isOldPasswordNotValid = true;
